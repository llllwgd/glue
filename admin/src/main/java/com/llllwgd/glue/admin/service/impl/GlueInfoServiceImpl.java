package com.llllwgd.glue.admin.service.impl;
...
import com.llllwgd.glue.admin.dao.ICodeLogDao;
import com.llllwgd.glue.admin.dao.IGlueInfoDao;
import com.llllwgd.glue.admin.dao.IProjectDao;
import com.llllwgd.glue.admin.service.IGlueInfoService;
...
public class GlueInfoServiceImpl implements IGlueInfoService {
    ...
    public ReturnT<String> update(GlueInfo codeInfo) {
        ...
        if (oldCodeInfo == null) {
            return new ReturnT<>(ReturnT.FAIL_CODE, "更新失败，GLUE记录不存在");
        }
    ...
    public ReturnT<String> updateCodeSource(CodeLog codeLog) {
        ...
        boolean b = XxlGlueBroadcaster.getInstance().procuceMsg(codeInfo_old.getName(), new HashSet<>(),System.currentTimeMillis());
        if (b) {
            log.info("更新脚本成功");
        } else {
            log.error("更新脚本失败");
